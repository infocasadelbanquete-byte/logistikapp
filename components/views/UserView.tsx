...
const UserView: React.FC = () => {
  ...
  const [formData, setFormData] = useState<Partial<User>>({ name: '', username: '', password: '', role: UserRole.STAFF, status: 'ACTIVO' });
  ...
  const handleCreateNew = () => { setEditingId(null); setFormData({ name: '', username: '', password: '', role: UserRole.STAFF, status: 'ACTIVO' }); setIsModalOpen(true); };
  const handleEdit = (user: User) => { setEditingId(user.id); setFormData({ name: user.name, username: user.username, password: user.password, role: user.role, status: user.status || 'ACTIVO' }); setIsModalOpen(true); };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.name && formData.username && formData.password) {
      const userToSave: User = { id: editingId || '', name: formData.name, username: formData.username, password: formData.password, role: formData.role as UserRole, status: formData.status as any || 'ACTIVO', lastActive: editingId ? undefined : new Date().toISOString() };
  ...
  const confirmApproval = async (role: UserRole) => {
      ...
      await storageService.saveUser({ ...approvalUser, role, status: 'ACTIVO' });
