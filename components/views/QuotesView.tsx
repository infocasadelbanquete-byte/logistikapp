...
const QuotesView: React.FC = () => {
  ...
  useEffect(() => {
    const unsub = storageService.subscribeToEvents((all) => {
        setQuotes(all.filter(e => String(e.status).toUpperCase() === 'QUOTE' || String(e.status).toUpperCase() === 'PROFORMA'));
    });
  ...
  const handleConfirm = async (q: EventOrder) => {
      if (await uiService.confirm("Confirmar Pedido", `¿Convertir PRO-${q.orderNumber} en un pedido confirmado?`)) {
          await storageService.saveEvent({ ...q, status: EventStatus.CONFIRMED });
          uiService.alert("Éxito", "Proforma confirmada.");
      }
  };

  const filteredQuotes = quotes.filter(q => q.clientName.toLowerCase().includes(searchQuery.toLowerCase()) || String(q.orderNumber).includes(searchQuery));
    ...
    <div className="space-y-6 animate-fade-in">
        ...
        <div className="bg-white p-4 rounded-2xl shadow-soft border border-zinc-100">
            <div className="relative">
                <input className="w-full bg-zinc-50 border-none p-3 pl-12 rounded-xl text-xs font-bold outline-none" placeholder="Buscar por cliente o número..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 pb-20">
            {filteredQuotes.map(q => (
                <div key={q.id} className="bg-white p-6 rounded-[2rem] shadow-soft border border-zinc-100">
                    ...
                    <div className="flex gap-2">
                        ...
                        <button className="w-10 h-10 bg-zinc-50 rounded-lg flex items-center justify-center">✏️</button>
                    </div>
                </div>
            ))}
        </div>
