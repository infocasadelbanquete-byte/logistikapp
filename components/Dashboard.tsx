...
const Dashboard: React.FC = () => {
  ...
  const weeklyOrders = events.filter(e => {
    ...
    // Excluir proformas y cancelados del flujo semanal operativo
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-4">
          <h2 className="text-xl font-black text-brand-950 uppercase tracking-tighter flex items-center gap-2">
            <span>📅</span> Agenda Semanal de Pedidos
          </h2>
          <div className="bg-white rounded-[2.5rem] shadow-premium border border-zinc-100 overflow-hidden">
            <div className="p-6 border-b border-zinc-50 bg-zinc-50/30 flex justify-between items-center">
              <span className="text-[10px] font-black text-zinc-400 uppercase tracking-widest">Próximas Entregas / Retiros</span>
              ...
              {weeklyOrders.map(e => (
                <div key={e.id} className="p-6 hover:bg-zinc-50 transition-colors flex items-center justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <p className="text-[9px] font-bold text-zinc-400 uppercase tracking-wider">🗓️ {e.executionDate} • {e.requiresDelivery ? 'ENVÍO A DOMICILIO' : 'RETIRO EN BODEGA'}</p>
                    {e.warehouseExitNumber && <span className="text-[8px] font-black text-brand-600 bg-brand-50 px-1.5 py-0.5 rounded uppercase mt-1 inline-block">EB N°: {e.warehouseExitNumber}</span>}
                  </div>
        ...
        <div className="space-y-6">
          <div className="bg-rose-50 border border-rose-100 rounded-[2rem] p-6 shadow-sm">
            <h3 className="text-rose-900 text-xs font-black uppercase mb-4 flex items-center gap-2">⚠️ Novedades en Ingresos</h3>
